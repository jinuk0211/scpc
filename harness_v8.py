...
def _final_state(task: dict[str, Any]) -> dict[str, Any]:
    ...
    # --- record-recency semantics from route_binding_order ---------------------
    # The generator's focal machinery already uses route_binding_order to select
    # the LATEST phase (see focal_resolution_trace.latest_phase_rule). The same
    # recency principle applies to control/target: a pending authority check that
    # is OLDER than a later boundary/candidates record must not drive ask/hold.
    # dev only contains orders where the fixed priority coincides with recency;
    # screening introduces candidates_after_authority where they diverge.
    rb = state.get("route_binding_order")
    authority_is_stale = rb in ("boundary_after_authority", "candidates_after_authority")
    if authority_is_stale and state.get("authority") in ("authority_incomplete", "user_binding_pending"):
        state["stale_pending_authority"] = state["authority"]
        state["authority"] = None
    candidates_stale = rb == "authority_after_candidates"
    if candidates_stale and state.get("route_candidate_snapshot") == "local_candidate_only":
        state["route_candidate_snapshot"] = None
    state["candidates_governing"] = rb == "candidates_after_authority"
...
def _final_is_memory_write(state: dict[str, Any]) -> bool:
    ...
    if state.get("route_candidate_snapshot") == "local_candidate_only":
        return True
    if state.get("candidates_governing") and state.get("route_candidate_snapshot") == "single_internal_candidate":
        # candidates snapshot is the latest routing record and it resolved to a
        # single internal candidate -> local/internal update flow.
        return True
...
def _final_clarify_reason(state: dict[str, Any], target: str) -> str | None:
    ...
    if (
        state.get("route_candidate_snapshot") == "mixed_local_external_candidates"
        and state.get("share_boundary_update") != "local_update_boundary"
    ):
        # latest routing record says local/external candidates are still mixed
        return "route_resolution_required"
...
def _final_user_response(control: str, target: str) -> str:
    if control == "hold":
        return (
            "전제 조건이 무효화되어 요청 실행을 보류합니다. 보안·동의·정책 상태를 확인하기 전에는 진행하지 않습니다. "
            "I will hold this request because a safety, consent, or policy precondition was invalidated."
        )
    if control == "ask":
        return (
            "대상 또는 허용 범위가 아직 확정되지 않아 실행 전에 사용자 확인이 필요합니다. "
            "I need user confirmation to resolve the target and scope before proceeding."
        )
    if control == "amend":
        return (
            f"민감 정보를 제외한 요약만 {target}(으)로 전달합니다. "
            f"I will redact sensitive fields and dispatch only a redacted summary to {target}."
        )
    if target == FINAL_LOCAL_TARGET:
        return (
            "외부 공유 없이 기기 내부 상태만 갱신합니다. "
            "I will proceed with a local status-only update in memory_store, without external dispatch."
        )
    return (
        f"요청을 승인된 범위로 진행하여 {target}(으)로 전달합니다. "
        f"I will proceed and dispatch to {target} within the permitted scope."
    )
...
class FinalHarness:
    ...
    def answer_task(self, task: dict[str, Any], session: dict[str, Any]) -> dict[str, Any]:
        ...
        if _final_local_update_override(task):
            ...
            if (
                state.get("share_boundary_update") == "redacted_after_selection_boundary"
                and state.get("route_binding_order") == "boundary_after_authority"
                and _final_is_external_target(target)
            ):
                reason = None  # latest boundary mandates redacted dispatch; overrides soft clarify
            hard_reason = bool(
                state.get("target_changed_after_turn")
                or state.get("duration_ambiguous")
                or state.get("payment_policy")
                or state.get("memory_conflict")
                or state.get("calendar_conflict")
            )
            if (
                (
                    (
                        state.get("authority") == "local_authority_confirmed"
                        and state.get("share_boundary_update") not in ("redacted_external_boundary", "redacted_after_selection_boundary", "dispatch_blocked_until_binding")
                    )
                    or (
                        state.get("candidates_governing")
                        and state.get("route_candidate_snapshot") in ("local_candidate_only", "single_internal_candidate")
                    )
                )
                and not hard_reason
            ):
                # local_authority_confirmed legitimizes a device-local flow
                # (distinct from internal_binding_confirmed, which binds an
                # internal *route*): complete as a local status update.
                target = FINAL_LOCAL_TARGET
                is_memory_write = True
                control = "proceed"
                scope = _final_content_scope(focal_obj, state, "local")
                ...
                # boundary record is the latest routing signal and it mandates
                # post-selection redaction -> minimal-disclosure dispatch.
                control = "amend"
                contains = _final_sensitive_tokens(focal_obj)
                excluded = _final_sensitive_exclusions(set(contains)) or ["raw_quote"]
                remove = "sensitive_fields" if len(excluded) > 1 else excluded[0]
                scope = {"mode": "redacted", "allowed_fields": ["summary"], "excluded_fields": excluded, "requires_user_confirmation": True}
                events = [
                    _final_event("read", focal_id, {"purpose": "minimal_disclosure"}),
                    _final_event("redact", focal_id, {"remove": remove}),
...
submission_payload = run_harness(screening_tasks, FinalHarness, harness_name="local_exam_harness_v8")
